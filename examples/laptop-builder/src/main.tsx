 ...
 *   - Cascading CSelect (CPU → compatible motherboards)
 *   - CValidate warning mode
 *   - CBOMEntry-style rollup via CSideEffect + NamedAdder
 *   - CLinearAggregation for live price totalling
 *   - CWorkbench showing configurator + BOM + problems + live price
 */
...
function partCase(code: string, label: string, price: number): Case {
    return ccase(
        code,
        `${label} — $${price}`,
...
function asDefault(c: Case, idx: number): Case {
    return idx === 0 ? cdefault(c) : c;
}

const ConfigurationType = CPanel(
    { header: "💻 Build your laptop", collapsible: false },
    CGroup([
        cmember("name", "Build name", CString({ defaultValue: "My build" })),

        cmember(
            "cpu",
            "Processor",
            CSelect([
                cdefault(partCase("i5", CPUS.i5!.label, CPUS.i5!.price)),
                partCase("i7", CPUS.i7!.label, CPUS.i7!.price),
                partCase("i9", CPUS.i9!.label, CPUS.i9!.price)
            ])
        ),
        ...
        (groupCtx: Ctx) => {
            const groupVal = groupCtx.value as
                | { cpu?: { $case?: string } }
                | undefined;
            const cpuCode = groupVal?.cpu?.$case ?? "i5";
            ...
            return cmember(
                ...
                CSelect(
                    cpu.chipsets.map((code, i) => {
                        ...
                        return asDefault(
                            partCase(code, info.label, info.price),
                            i
                        );
                    })
                )
        ...
        cmember(
            ...
            CValidate(
                (_node, { warning }, ctx) => {
                    const v = (
                        ctx.value as { $case?: string } | undefined
                    )?.$case;
                    if (v === "16") {
                        warning(
...
const LaptopType = CLinearAggregation(
    "price",
    SimpleAdder,
    CLinearAggregation(
        "bom",
